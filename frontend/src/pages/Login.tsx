...
import { useUser, USERS, Role } from '../context/UserContext';
...
export default function Login() {
  ...
  function submit() {
    ...
    // mockup: map known emails to roles
    const found = Object.values(USERS).find(u => u.email === email);
    if (found && pass.length >= 6) { setUser(found); nav('/lobby'); }
    else setError('Credenciais inválidas — usa um dos utilizadores demo à esquerda.');
  }
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1px 1fr', gap:34, alignItems:'start', maxWidth:780, margin:'0 auto' }}>
          ...
          <div>
            ...
            <div style={{ marginBottom:15 }}>
              ...
              <input className="input-base input-mono" value={email} onChange={e => setEmail(e.target.value)} placeholder="jane.smith@horizonlabs.io" />
            </div>
            <div>
            ...
            <button onClick={submit} className="btn-primary hover-bright" style={{ width:'100%', fontSize:13.5, padding:12, borderRadius:9, marginTop:18 }}>Entrar →</button>
